...
define('PLUPLOAD_MIME_ERR', 106);
define('PLUPLOAD_CONVERT_ERR', 107);
...
class PluploadHandler {
    ...
    PLUPLOAD_MIME_ERR => "File mime type not allowed.",
    PLUPLOAD_CONVERT_ERR => "Bild konnte nicht konvertiert werden.",
	...
	static function handle($conf = array())
	{
      ...
      if ($conf['allow_mime_type']) {
        if(is_string($conf['allow_mime_type'])) {

          $imgInfo = getImageSize($_FILES[$conf['file_data_name']]["tmp_name"]);
          if(strpos($imgInfo['mime'], $conf['allow_mime_type']) !== false) {
            $_FILES[$conf['file_data_name']]["tmp_name"];
            $convertToJPG = false;
            /*
 * Wenn kein JPEG dann Konvertieren
 */
            if($imgInfo[2] != 2) {

              $fileToConvert = $_FILES[$conf['file_data_name']]["tmp_name"];
              $noRGB = exec("nconvert -quiet -info \"" . $fileToConvert . "\" | egrep -o 'CMYK'");
              /*
               * PSD CMYK Check dann nur mir convert in RGB wandeln.
               */
              if($noRGB == "CMYK") {
                $imgInfo['channels'] = 4;
              } else {
            ...
            /*
              * Wenn CMYK Bild erst mit Convert und entsprechendem profil konvertieren
             */
            if($imgInfo['channels'] === 4) {
              $ICC_profile["srgb"] = "/usr/lib/pixelprint/icc/sRGB_IEC61966-2-1.icc";
              $ICC_profile["cmyk"] = "/usr/lib/pixelprint/icc/EuropeISOCoatedFOGRA27.icc";
              $fileToConvert = $_FILES[$conf['file_data_name']]["tmp_name"];
              rename($fileToConvert, $fileToConvert . "_cmyk");
              $ret = exec("convert -profile " . $ICC_profile["cmyk"] . " -profile " . $ICC_profile["srgb"] . " \"" . $fileToConvert . "_cmyk\" $fileToConvert  2>&1");

            }

            if($convertToJPG == true) {
              $fileToResize =  $_FILES[$conf['file_data_name']]["tmp_name"];
              rename($fileToResize, $fileToResize . "_orig");
              $ret = exec("nconvert -quiet -no_auto_ext -merge_alpha -truecolors -ctype rgb -q 95 -out jpeg -o \"" . $fileToResize . "\" \"" . $fileToResize . "_orig\" 2>&1");
              $new_imageinfo=getImageSize($fileToResize);

              if(!is_file($fileToResize) || $new_imageinfo["mime"]!="image/jpeg"){
                throw new Exception('', PLUPLOAD_CONVERT_ERR);
              }
            }

          } else {
            throw new Exception('', PLUPLOAD_MIME_ERR);
